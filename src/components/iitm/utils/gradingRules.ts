...
/**
 * "blocking"   - the student cannot obtain a grade this term and must repeat the
 *                course (doc: ineligible for the end term => WA/WQ, "repeat the
 *                entire course"). Score is capped at 39.
 * "incomplete" - everything is fine except a programming exam. The document is
 *                explicit: "If you have failed in the OPPE but wrote the ET exam
 *                and crossed the cutoff for T ... grade = I_OP", and the student
 *                may "register to do the OPPE alone ... All other mark components
 *                of Final course score will be taken from the previous run."
...
type Requirement =
  | { type: "minimum"; field: string; minimum: number; label: string; severity?: IssueSeverity }
  // "at least one of these fields must reach `minimum`" - e.g. the published
  // rule "score in ONE of the two programming exams (OPPE1, OPPE2) >= 40".
  ...
  | { type: "any-minimum"; fields: string[]; minimum: number; label: string; severity?: IssueSeverity }
...
export const FORMULA_PROFILES: Record<string, FormulaProfile> = {
  ...
  "foundation-programming": {
    ...
    requirements: [{ type: "minimum", field: "GAA", minimum: 40, label: "Assignment eligibility average" }, { type: "any-eligible", fields: ["OPPE1_EL", "OPPE2_EL"], label: "eligible for at least one OPPE" }, { type: "any-minimum", fields: ["OPPE1", "OPPE2"], minimum: 40, label: "at least one OPPE (OPPE1 or OPPE2)", severity: "incomplete" }],
  },
  "c-programming-2026": {
    ...
    requirements: [{ type:"one-present", fields:["Qz1"], label:"Quiz 1" }, { type:"any-minimum", fields:["OPPE1","OPPE2"], minimum:40, label:"at least one OPPE (OPPE1 or OPPE2)", severity: "incomplete" }],
  },
  ...
  "mlp": { formula: "0.1GAA + 0.3F + 0.2OPPE1 + 0.2OPPE2 + 0.2KA", calculate: v => 0.1*n(v,"GAA") + 0.3*n(v,"F") + 0.2*n(v,"OPPE1") + 0.2*n(v,"OPPE2") + 0.2*n(v,"KA"), requirements: [{ type: "minimum", field: "GAA", minimum: 40, label: "Assignment eligibility average" }, { type: "any-minimum", fields: ["OPPE1", "OPPE2"], minimum: 40, label: "at least one OPPE (OPPE1 or OPPE2)", severity: "incomplete" }] },
  "bdm": { formula: "0.05GA + max(0.6F + 0.25max(Qz1,Qz2), 0.4F + 0.25Qz1 + 0.3Qz2)", calculate: v => 0.05*n(v,"GA") + Math.max(0.6*n(v,"F") + 0.25*maxQuiz(v), 0.4*n(v,"F") + 0.25*n(v,"Qz1") + 0.3*n(v,"Qz2")), requirements: [{ type: "minimum", field: "GA", minimum: 40, label: "Assignment eligibility average" }, { type: "one-present", fields:["Qz1","Qz2"], label:"at least one quiz" }] },
  ...
  "dbms": { formula: "0.03GAA2 + 0.02GAA3 + 0.2OP + 0.45F + max(0.2max(Qz1,Qz2), 0.1Qz1 + 0.2Qz2)", calculate: v => 0.03*n(v,"GAA2")+0.02*n(v,"GAA3")+0.2*n(v,"OP")+0.45*n(v,"F")+Math.max(0.2*maxQuiz(v),0.1*n(v,"Qz1")+0.2*n(v,"Qz2")), requirements:[{type:"minimum",field:"GAA",minimum:40,label:"Assignment eligibility average"},{type:"one-present",fields:["Qz1","Qz2"],label:"at least one quiz"},{type:"minimum",field:"OP",minimum:35,label:"OPPE",severity:"incomplete"}] },
  "java": { formula: "0.05GAA + 0.2max(PE1,PE2) + 0.45F + max(0.2max(Qz1,Qz2), 0.1Qz1 + 0.2Qz2) + 0.1min(PE1,PE2)", calculate:v=>0.05*n(v,"GAA")+0.2*Math.max(n(v,"PE1"),n(v,"PE2"))+0.45*n(v,"F")+Math.max(0.2*maxQuiz(v),0.1*n(v,"Qz1")+0.2*n(v,"Qz2"))+0.1*Math.min(n(v,"PE1"),n(v,"PE2")), requirements:[...standardRequirements,{type:"any-minimum",fields:["PE1","PE2"],minimum:30,label:"at least one programming exam (PE1 or PE2)",severity:"incomplete"}] },
  "system-commands": { formula:"0.05GAA + 0.25Qz1 + 0.3OPPE + 0.3F + 0.1BPTA", calculate:v=>0.05*n(v,"GAA")+0.25*n(v,"Qz1")+0.3*n(v,"OPPE")+0.3*n(v,"F")+0.1*n(v,"BPTA"), requirements:[{type:"minimum",field:"GAA",minimum:40,label:"Assignment eligibility average"},{type:"any-eligible",fields:["OPPE_EL"],label:"eligible for the OPPE"},{type:"minimum",field:"OPPE",minimum:40,label:"OPPE",severity:"incomplete"}] },
  ...
  "ds-ai-lab": { formula:"0.05GAA + 0.25Qz2 + 0.4P + 0.3V + Bonus", calculate:v=>0.05*n(v,"GAA")+0.25*n(v,"Qz2")+0.4*n(v,"P")+0.3*n(v,"V")+n(v,"Bonus"), requirements:[{type:"minimum",field:"GAA",minimum:40,label:"Assignment eligibility average"},{type:"minimum",field:"V",minimum:55,label:"Viva"}],capAt100:true },
...
/** Builds the message for a component that fell short of its threshold. */
function shortfall(label: string, minimum: number, severity: IssueSeverity = "blocking"): EligibilityIssue {
  ...
  return { severity, message: `${label} must be at least ${minimum}.` };
}
...
export function getEligibilityIssue(subjectKey: string, values: Values): EligibilityIssue | null {
  ...
  for (const requirement of profile.requirements || []) {
    if (requirement.type === "minimum") {
      if (n(values, requirement.field) < requirement.minimum) {
        const issue = shortfall(requirement.label, requirement.minimum, requirement.severity);
      ...
      if (!requirement.fields.some((field) => isEligible(values, field))) {
        return { message: `You must be ${requirement.label} to appear for the end term exam.`, severity: "blocking" };
      }
